...
Mesh 클래스를 완성하기 위해 마지막으로 정의해야 할 함수는 Draw 함수입니다. 메시를 렌더링하기 전에 `glDrawElements`를 호출하기 전에 적절한 텍스처를 바인딩해야 합니다. 하지만 메시가 어떤 텍스처를 가지고 있는지(혹은 아무 텍스처도 가지고 있지 않은지) 그리고 어떤 유형의 텍스처를 가지고 있는지 처음부터 알 수 없기 때문에 이 과정은 다소 어렵습니다. 그렇다면 셰이더에서 텍스처 유닛과 샘플러를 어떻게 설정해야 할까요?
