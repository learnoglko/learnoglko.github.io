...
이제 카메라를 어느 정도 움직일 수 있을 것입니다. 다만, 움직임 속도는 시스템에 따라 다르므로 cameraSpeed ​​값을 조정해야 할 수도 있습니다.

!!! note ""
    우리는 아직 델타타임(FPS와 상관없이 값을 변환할 수 있게 해주는 기술) 또는 그와 유사한 기술을 사용하지 않기 때문에, 시스템 FPS에 따라 이동 속도에 차이가 생깁니다.
